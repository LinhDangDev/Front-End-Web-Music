...
class FeaturedCreators extends React.Component {
    ...
    componentDidMount() {
        ...
        AlbumService.getAll().then((res) => {

            // sort from newest to oldest
            DISPLAY_ALBUMS = ALBUMS = (res.result).sort((a, b) => Date(b.releaseDate) - Date(a.releaseDate));
                            ...
                            {group.map((item) => (
                                <div className="card-simple" key={item.albumId}>
                                    <Link to={`/album/${item.albumId}`}>
                                        <figure>
                                            <img src={`http://localhost:8080/img/${item.coverImage}`} alt={item.albumName} />
                                        </figure>
